<template>
  <div class="bg-black c-white h-client section-wrapper" style="position: relative">
    <div class="snmk-container w-100 section-text-box">
      <div class="snmk-col-10 snmk-col-l-10 snmk-col-m-8 snmk-col-s-4 snmk-col-xs-4  snmk-col-offset-1 snmk-col-l-offset-1 vertical-center">
      ...
      <div class="snmk-col-3 snmk-col-l-3 snmk-col-m-3 snmk-col-s-4 snmk-col-xs-4  snmk-col-offset-1 snmk-col-l-offset-1 vertical-center">
        <div>
          ...
          <template v-for="(item, index) of dataSrc.displays">
            <div class="text-center mob-title my-m"  v-if='isMobile && index === currDisplayIdx'  :key="item.text+'text'" v-html='item.text'></div>
            <div :key="item.text" v-if='isMobile && index === currDisplayIdx' class="title-4 sf-desc" v-html='item.desc'></div>
          </template>
        </div>
      </div>
      <div class="snmk-col-6 snmk-col-l-6 snmk-col-m-5 snmk-col-s-4 snmk-col-xs-4 snmk-col-offset-1 snmk-col-l-offset-1 pos-relative" v-if="!isMobile">
        <template v-for="(item, index) of dataSrc.displays" :key="item.text">
          ...
          <img v-else
            v-lazy="isMobile ? item.imgSrcMob :item.imgSrc " :alt="isMobile ? item.mobAlt : item.alt"
            class="displays-img" :class="{'opacity fade-in': index === currDisplayIdx}"
            :style="{ position: index === 0 ? 'relative' : 'absolute' }"
          >
        </template>
