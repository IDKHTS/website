<template>
  <div class="bg-black c-white h-client section-wrapper" style="position: relative">
    <div class="snmk-container w-100 section-text-box">
      <div
        class="snmk-col-6 snmk-col-l-6 snmk-col-m-5 snmk-col-s-4 snmk-col-xs-4 snmk-col-offset-1 snmk-col-l-offset-1 pos-relative"
        v-if="!isMobile">
        <template v-for="(item, index) of dataSrc.displays" :key="item.text">
          <video v-if='item.videoWebm || item.videonMp4' :ref="el => mapVideo(el, index)" :key="item.text" muted
            :poster="item.poster" :autoplay="false" :preload="'metadata'" loop webkit-playsinline playsinline
            x5-video-player-type="h5-page" class="displays-img" :class="{ 'opacity fade-in': index === currDisplayIdx }"
            :style="{ position: index === 0 ? 'relative' : 'absolute' }">
            <source :src="item.videoWebm">
            <source :src="item.videonMp4">
          </video>
          <NuxtImg v-else :src="isMobile ? item.imgSrcMob : item.imgSrc" :alt="isMobile ? item.mobAlt : item.alt"
      ...
      <div
        class="snmk-col-4 snmk-col-l-4 snmk-col-m-3 snmk-col-s-4 snmk-col-xs-4  snmk-col-offset-1 snmk-col-l-offset-1 vertical-center">
        <div>
          <div class="first-title bold-700" :class="{ 'text-center': isMobile }" v-if='dataSrc.title'
            v-html="isMobile ? dataSrc.title.replace('<br>', '') : dataSrc.title"></div>
          <!-- data-aos='fade-up' -->
          <div v-if="!isMobile" class="mt-xl display-flex">
            ...
            <div class="checkout-nav-bar-wrapper">
              <div v-for="(item, index) of dataSrc.displays" :key="item.text" @click="selectIndex(index)"
                class="checkout-nav-bar" :class="{ 'checkout-nav-active-bar': index === currDisplayIdx }">
              </div>
            </div>
            <div class="ml-l">
              <div v-for="(item, index) of dataSrc.displays" :key="item.text" @click="selectIndex(index)"
                class="mt-m checkout-nav title-4" :class="{ 'checkout-nav-active bold-700': index === currDisplayIdx }">
                {{ item.text }}
                <div class="title-4 sf-desc mt-m" :ref="el => mapDescEl(el, index)">{{ item.desc }}</div>
              </div>
            </div>
          </div>
          <template v-for="(item, index) of dataSrc.displays">
            <div class="text-center mob-title my-m" v-if='isMobile && index === currDisplayIdx' :key="item.text + 'text'"
              v-html='item.text'></div>
            <div :key="item.text" v-if='isMobile && index === currDisplayIdx' class="title-4 sf-desc" v-html='item.desc'>
            </div>
          </template>
        </div>
      </div>
    </div>
    <Swiper v-if="isMobile" @changeSlide="changeSlide" :options="{
      autoplay: {
        delay: 5000,
        stopOnLastSlide: false,
        disableOnInteraction: true
      }
    }" :swiperId="screenId + 'swiper'" class="section-img-box w-100 h-100" @loaded='mapSwiper'>
    ...
    <CheckoutNavMob @selectIndex="selectIndex" :currDisplayIdx="currDisplayIdx" :options="dataSrc"
      class="mt-3xl display-flex" :withTitle='false'></CheckoutNavMob>
  </div>
</template>
