  ...
  <div class="text-video bg-black-2 c-white section-wrapper pos-relative">
    <div class="snmk-container vertical-center py-3xl" ref="text-container">
      <template v-if='!isAssetLeft'>
        <div class="snmk-col-4 snmk-col-l-4 snmk-col-m-4 snmk-col-s-4 snmk-col-xs-4 snmk-col-offset-1 snmk-col-l-offset-1" :style='{"padding-right": isMobile ? "" : "48px"}' >
        ...
        <div class="snmk-col-6 snmk-col-l-6 snmk-col-m-4 snmk-col-s-4 snmk-col-xs-4 pos-relative" :class="{'mt-m': isMobile}">
          <template v-if='dataSrc.videoWebm || dataSrc.videonMp4'>
            ...
            <template v-else>
              <NuxtImg :src="dataSrc.gifSrc" :alt="dataSrc.title" loading='lazy'/>
            </template>
          </template>
          <template v-else>
              <NuxtImg :src="!isMobile ? dataSrc.imgSrc : dataSrc.imgMobSrc" :alt="dataSrc.title" loading='lazy'/>
          </template>
        </div>
      ...
      <template v-else>
        ...
        <div class="snmk-col-4 snmk-col-l-4 snmk-col-m-4 snmk-col-s-4 snmk-col-xs-4"  :style='{"padding-left": isMobile ? "" : "48px"}'>
          <div class="title-1 bold-700 mb-m" :class="{'text-center': isMobile}" v-html="dataSrc.title"></div>
          <div v-if='!isMobile' class="title-4" v-html="dataSrc.desc" ></div>
        </div>
...
const initVideo = () => {
  ...
  videoEl.value.addEventListener('ended', (event: Event) => {
