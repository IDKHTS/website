<template>
  <div class="text-video bg-black-2 c-white section-wrapper pos-relative" :style="{'min-height': `${viewerOptions.useViewer && isMobile ? '' : 'unset'}`}">
    ...
    <div :class="{'img-box': viewerOptions.useViewer, 'no-viewer': !viewerOptions.useViewer}" ref="adapterEl" :style="imgBoxStyle">
...
const imgAdapteWidth = () => {
  ...
  imgBoxStyle.value.width = imgViewWidth.value + 'px'
}
const setHeight = () => {
  if ((!isMobile.value || !!imgBoxStyle.value.height) && props.viewerOptions.useViewer) return
  ...
  if (!props.viewerOptions.useViewer && !isMobile.value) {
    ...
    setTimeout(() => {
      ...
      if ((el.videoHeight === 0 || el.videoWidth === 0)) {
        ...
        el.addEventListener('canplaythrough', (event: Event) => {
    ...
    adapter.style.height = `${window.innerHeight - textH - $(adapter).outerHeight(true) + $(adapter).height()}px`
  }
}
const initVideo = _.throttle(() => { imgAdapteWidth();setHeight() }, 100)
onMounted(() => {
  props.isVideoLeft ? (imgBoxStyle.value.left = 0) : (imgBoxStyle.value.right = 0)
  initVideo()
...
const adapterHeight = (height: number) => {
  imgBoxStyle.value.height = height + 'px'
}
</script>
