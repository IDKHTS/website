...
var setting = price(new Date().getTime());
  ...
  },
  {
    ...
    "shippingFee" : 32
      ...
      countryElement.onchange = function () {
        shipping_fee = MAXIMUM_FEE;

        var countryName = countryElement.value;

        for (var crsIdx = 0; crsIdx < shipping_data.length; crsIdx ++) {
          if (countryName === shipping_data[crsIdx].countryName) {
            console.log("Found shipping fee for : " + countryName + " | fee = " + shipping_data[crsIdx].shippingFee);
            shipping_fee = shipping_data[crsIdx].shippingFee;
            break;
          }
        }
        ...
        // set display
        document.getElementById('shipping_display').innerHTML = shipping_fee;
        document.getElementById('total_display').innerHTML = shipping_fee + setting.price;
        document.getElementById('stripePayBtn').value = "Pay $" + (shipping_fee + setting.price) + " with Credit Card";

        // set value
        document.getElementById('shipping').value = shipping_fee;
        document.getElementById('shipping2').value = shipping_fee;
        document.getElementById('stripeShipping').value = shipping_fee;

        // remove alsert class
        const el = document.getElementsByClassName('crs-country')[0];
        el.classList.remove('alert');

        const exclusiveAgencyDict = {
            'Hong Kong': 'everbest',
            'Macao': 'everbest'
        };

        // publish event for exclusive agency
        const agencyName = exclusiveAgencyDict[countryName];
        if (agencyName) {
          PubSub.publish('ExclusiveAgency', { countryName: countryName, agencyName: agencyName });
          // Erase price info
          document.getElementById('shipping_display').innerHTML = '-';
          document.getElementById('total_display').innerHTML = '-';
        } else {
          PubSub.publish('ExclusiveAgency', null);
        }
      }
      ...
      //     shipping_fee = MAXIMUM_FEE; // reset
      //   };
